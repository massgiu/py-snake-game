...
class Snake(object):
    ...
    def __init__(self, color):  # pos: head position
        self.color = color
    ...
    def addCube(self):
        ...
        if direction == "RIGHT":  # going right (x-1, same y)
            self.body_list.append(Cube((tail.pos[0] - Constants.SIZE_CUBE, tail.pos[1])))
        elif direction == "LEFT":  # going left (x+1, same y)
            self.body_list.append(Cube((tail.pos[0] + Constants.SIZE_CUBE, tail.pos[1])))
        elif direction == "DOWN":  # going down (same x, y-1)
            self.body_list.append(Cube((tail.pos[0], tail.pos[1] - Constants.SIZE_CUBE)))
